...
public class TickTrackerOverlay extends OverlayPanel
{
	...
	private void drawExtraInformation(Graphics2D graphics)
	{
		panelComponent.getChildren().addAll(asList(TitleComponent.builder().text("Ticks").build(),
			...
			LineComponent.builder().right(String.valueOf((long)(plugin.getRunningTickAverageNS().getMean() / plugin.getNANOS_PER_MILLIS()))).left("Tick Average ms").build(),
			LineComponent.builder().right(String.format("%.2f", plugin.getRunningTickAverageNS().getStandardDeviation() / plugin.getNANOS_PER_MILLIS())).left("Tick STDEV ms").build()));
